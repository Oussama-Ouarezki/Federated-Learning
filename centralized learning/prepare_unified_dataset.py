...
IMG_SIZE = 32  # Using 32x32 grayscale images
TEST_SPLIT = 0.15  # 15% for test (to match the prepare script's split)
...
class UnifiedArabicDataset(Dataset):
    ...
    def __init__(self, metadata_csv, images_dir, transform=None):
        ...
        self.image_paths = []
        self.labels = []
        self.datasets = []  # Track which dataset each image is from
...
class ArabicLetterCNN(nn.Module):
    """
    CNN model for Arabic Letter Recognition
    Architecture for 32x32 grayscale images
    
    Block 1: Conv(1 → 32, 3×3) → ReLU → BatchNorm → MaxPool(2×2) → 16×16×32
    Block 2: Conv(32 → 64, 3×3) → ReLU → BatchNorm → MaxPool(2×2) → 8×8×64
    Block 3: Conv(64 → 128, 3×3) → ReLU → BatchNorm → MaxPool(2×2) → 4×4×128
    ...
    def __init__(self, num_classes):
        ...
        # Feature size: 128 channels * 4 * 4 = 2048
...
def plot_metrics(train_losses, train_accs, val_losses, val_accs):
    ...
    plt.tight_layout()
    plt.savefig('training_metrics_unified.png', dpi=300, bbox_inches='tight')
    print("✓ Metrics plot saved to 'training_metrics_unified.png'")
    plt.close()
...
def plot_confusion_matrix(cm, class_names):
    ...
    plt.tight_layout()
    plt.savefig('confusion_matrix_unified.png', dpi=300, bbox_inches='tight')
    print("✓ Confusion matrix saved to 'confusion_matrix_unified.png'")
    plt.close()
...
def plot_per_class_accuracy(all_labels, all_preds, class_names):
    ...
    plt.tight_layout()
    plt.savefig('per_class_accuracy_unified.png', dpi=300, bbox_inches='tight')
    print("✓ Per-class accuracy plot saved to 'per_class_accuracy_unified.png'")
    plt.close()

def main():
    print("\n" + "="*80)
    print("UNIFIED ARABIC LETTER TRAINING - 32x32 GRAYSCALE - 3 DATASETS COMBINED")
    print("="*80)
    print(f"1. Loading unified dataset from: {DATA_DIR}")
    print("2. Reading metadata from CSV")
    print("3. Creating train/val/test splits")
    print("4. Applying augmentation (NO horizontal flip)")
    print("5. Training with warmup + cosine annealing")
    print("6. Using 32x32 grayscale images")
    print("="*80 + "\n")
    ...
    # STEP 2: Get train/val/test indices BEFORE creating datasets
    ...
    # Reduced augmentation intensity for 32x32 images
    train_transform = transforms.Compose([
        transforms.Resize((IMG_SIZE, IMG_SIZE)),
        # NO RandomHorizontalFlip - preserves Arabic letter orientation
        transforms.RandomRotation(10),  # ±10 degrees (reduced for smaller images)
        transforms.RandomAffine(
            degrees=0, 
            translate=(0.05, 0.05),  # ±5% translation (reduced for 32x32)
    ...
    print("✓ Created SEPARATE dataset instances for train/val/test")
    print("✓ Augmentation applied ONLY to training set")
    print("✓ NO horizontal flip - preserves Arabic letter orientation")
    print("✓ Validation and test sets use original non-augmented images\n")
    ...
    model = ArabicLetterCNN(num_classes).to(DEVICE)
    ...
    # Load best model
    checkpoint = torch.load('best_model_unified.pth')
    ...
    print("="*80)
    print("Final Evaluation on Test Set (Original Non-Augmented Data)")
    print("="*80)
    ...
    print("  - Unified dataset from 3 sources (AHCD, HMBD, OIHCBD)")
    print(f"  - Image size: 32x32 pixels (grayscale)")
    print("  - Architecture: 3 conv blocks (32->64->128 channels)")
    print("  - Training: Augmented data (rotation, affine, brightness)")
    print("  - Training: NO horizontal flip (preserves Arabic orientation)")
    print("  - Validation/Test: Original non-augmented data")
    print("  - Learning rate warmup + cosine annealing")
    print("  - Early stopping with patience")
    print("\nGenerated files:")
    print("  - training_metrics_unified.png (Loss & Accuracy plots)")
    print("  - confusion_matrix_unified.png (Detailed confusion matrix)")
    print("  - per_class_accuracy_unified.png (Per-class accuracy)")
    print("  - best_model_unified.pth (Best model checkpoint)")
    print("="*80)
